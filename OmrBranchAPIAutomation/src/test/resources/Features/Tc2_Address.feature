Feature: Address Module for API Automation
  ...
  Scenario Outline: User update the address from endpoints
    ...
    When User add the payload"<addressid>", "<firstname>","<lastname>","<mobileno>","<apartment>","<state>","<city>","<country>","<zipcode>","<address>","<addresstype>" for updating the address with address_Id
    ...
    Examples: 
      | addressid | firstname | lastname | mobileno   | apartment | state | city | country | zipcode | address                   | addresstype |
      |     17299 | captain   | sparrow  | 9884516467 | Carebean  |    33 | 3378 |     101 |  202020 | 3rd, waldormod, annanagar | work        |
